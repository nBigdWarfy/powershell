function Install-Pwsh {
    try {
        ...
        Write-Host "An error has occurred: Powershell could not be installed"
        exit 2
    }
    Write-Host "$($error[0])"
}
...
function Install-WinGet {
    try {
        ...
        if (Test-Path "HKLM:\SOFTWARE\Microsoft\PowerShell\1\") {
            Write-Host "An error occurred: the proccess could not been completed"
            exit 4
        } else {
            Write-Host "An error occurred: Powershell installation is required for this proccess"
            exit 3
        }
    }
    Write-Host "$($error[0])"
...
function Install-GitHub {
    try {
        ...
        if (Get-Command winget -ErrorAction SilentlyContinue) {
            ...
            Write-Host "An error occurred: Winget is not installed"
            exit 5 
        }
    }
    Write-Host "$($error[0])"
